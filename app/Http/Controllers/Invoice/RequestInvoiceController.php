...
class RequestInvoiceController extends Controller
{
    ...
    {
        $this->middleware('permission:Ver_FacturasSolicitadas|Crear_FacturasSolicitadas|Editar_FacturasSolicitadas|Eliminar_FacturasSolicitadas',['only' => ['index']]);
        $this->middleware('permission:Crear_FacturasSolicitadas',['only' => ['create', 'store']]);
        $this->middleware('permission:Editar_FacturasSolicitadas',['only' => ['edit', 'update']]);
        $this->middleware('permission:Eliminar_FacturasSolicitadas',['only' => ['destroy']]);
    }
    /**
    ...
    {              
        ...
        return view('admin.invoice.request.report');
    }
    ...
    public function cancel_advance(Request $request)
    {
        ...
        try {
            ...
            if ($errors==0) {
                ...
                DB::rollback();
                return response()->json('Error al denegar Facturas_Solicitadas:', 422);
            }
            
        }    
        catch (\Exception $e) {
            DB::rollback();
            return response()->json('Error al denegar Facturas_Solicitadas:'.$e, 422);
        }
    }
