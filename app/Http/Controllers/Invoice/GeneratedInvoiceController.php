...
class GeneratedInvoiceController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:Ver_FacturasGeneradas|Crear_FacturasGeneradas|Editar_FacturasGeneradas|Eliminar_FacturasGeneradas',['only' => ['index']]);
        $this->middleware('permission:Crear_FacturasGeneradas',['only' => ['create', 'store']]);
        $this->middleware('permission:Editar_FacturasGeneradas',['only' => ['edit', 'update']]);
        $this->middleware('permission:Eliminar_FacturasGeneradas',['only' => ['destroy']]);
    }
    /**
    ...
    {              
        ...
        return view('admin.invoice.generated.report');
    }
    ...
    public function cancel_advance(Request $request)
    {
        ...
        try {
            ...
            if ($errors==0) {
                ...
                DB::rollback();
                return response()->json('Error al denegar Facturas_Generadas:', 422);
            }
            
        }    
        catch (\Exception $e) {
            DB::rollback();
            return response()->json('Error al denegar Facturas_Generadas:'.$e, 422);
        }
    }
