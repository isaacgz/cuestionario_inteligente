...
class PendingInvoiceController extends Controller
{
    ...
    {
        $this->middleware('permission:Ver_FacturasPendientes|Crear_FacturasPendientes|Editar_FacturasPendientes|Eliminar_FacturasPendientes',['only' => ['index']]);
        $this->middleware('permission:Crear_FacturasPendientes',['only' => ['create', 'store']]);
        $this->middleware('permission:Editar_FacturasPendientes',['only' => ['edit', 'update']]);
        $this->middleware('permission:Eliminar_FacturasPendientes',['only' => ['destroy']]);
    }
    /**
    ...
    {              
        ...
        return view('admin.invoice.pendings.report');
    }
    ...
    public function cancel_advance(Request $request)
    {
        ...
        try {
            ...
            if ($errors==0) {
                ...
                DB::rollback();
                return response()->json('Error al denegar Facturas_Pendientes:', 422);
            }
            
        }    
        catch (\Exception $e) {
            DB::rollback();
            return response()->json('Error al denegar Facturas_Pendientes:'.$e, 422);
        }
    }
